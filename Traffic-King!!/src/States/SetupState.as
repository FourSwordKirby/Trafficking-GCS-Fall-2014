			...
			if (in_setup)
			{
				...
				{	
					...
					if ((FlxG.mouse.x-FlxG.camera.scroll.x) > ((Parameters.SCREEN_WIDTH - map.getMapWidth()) * zoom / 2) && 
						(FlxG.mouse.x-FlxG.camera.scroll.x) < ((Parameters.SCREEN_WIDTH - map.getMapWidth()) * zoom / 2 + map.getMapWidth()/2) &&
						(FlxG.mouse.y-FlxG.camera.scroll.y) > ((Parameters.SCREEN_HEIGHT - map.getMapHeight()) * zoom / 2) && 
						(FlxG.mouse.y-FlxG.camera.scroll.y) < ((Parameters.SCREEN_HEIGHT - map.getMapHeight()) * zoom / 2 + map.getMapHeight()/2)
						)
					{
						if (FlxG.mouse.justPressed())
						{
							...
							FlxG.camera.scroll.x = MouseRectangle.x;
							FlxG.camera.scroll.y = MouseRectangle.y;
